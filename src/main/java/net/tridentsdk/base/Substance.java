...
public enum Substance {
    ...
    static {
        Map<Integer, Substance> builder = Maps.newHashMap();
        for (Substance substance: EnumSet.allOf(Substance.class)) {
            builder.put(substance.idInt, substance);
        }
        ids = ImmutableMap.copyOf(builder);
    }

    Substance(int id, int stack, String data) {

        this.id = Integer.toString(id);
        this.idInt = id;
        this.maxStack = stack;
        this.data = data;
        this.replaced = PATTERN.matcher(this.id).replaceAll(" ");
    }

    Substance(int id, int stack) {
        this(id, stack, "");
    }
     ...
     */
    public static Substance getByName(String id) {
